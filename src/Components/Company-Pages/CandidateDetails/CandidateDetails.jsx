...
const CandidateDetails = () => {
  ...
  const applyFilters = () => {
    ...
    if (experience) {
      filtered = filtered.filter(
        (candidate) => candidate.candidate.yearsOfExperience >= experience
      );
    }

    if (skill) {
      filtered = filtered.filter((candidate) =>
        candidate.candidate.skills.includes(skill)
      );
    }
  ...
  useEffect(() => {
    if (!applications) dispatch(fetchApplications(id));
  }, [applications]);
  ...
  const acceptHandler = async (jobId, candidateId, status) => {
    try {
      const endpoint =
        status === "accept"
          ? `${BACKEND_URL}/interviewer/approve-candidate/${jobId}/${candidateId}`
          : `${BACKEND_URL}/interviewer/reject-candidate/${jobId}/${candidateId}`;
  ...
  const copyToClipboard = async (roomId) => {
    if (!roomId) {
      toast.error("No room code available to copy.");
      return;
    }
    ...
    try {
      ...
      toast.error("Failed to copy the room code. Please try again.");
    }
  };

  const getRoomCodeContent = (candidate) => {
    const currentRoomCode = rooms?.find(
      (room) => room.candidateId === candidate.candidate._id
    );

    if (!currentRoomCode?.roomId) {
      return <h5>Reload the page to get the room code.</h5>;
    }
    ...
    return (
      <>
        ...
        <button
          className="copy-code"
          onClick={() => copyToClipboard(currentRoomCode.roomId)}
        >
          Copy Room Code
        </button>
      </>
              ...
              <div className="participant-info">
                ...
                <ul className="event-time">
                  <li>
                    ...
                    <b>{candidate.candidate.yearsOfExperience} years</b>
                  </li>
                  <li>
                    <i className="fa-solid fa-clipboard"></i> Skills:{" "}
                    <b>{candidate.candidate.skills?.join(", ")}</b>
                  </li>
                </ul>
                <div className="buttons-div">
                  <button
                    className="resume-view"
                    onClick={() => handleClick(candidate.candidate.resume.url)}
                  >
                    View Resume
                  </button>
                  {candidate.status === "pending" ? (
                    <div>
                      <button
                        className="accept"
                        onClick={() =>
                          acceptHandler(candidate.job, candidate.candidate._id, "accept")
                        }
                      >
                      ...
                      <button
                        className="decline"
                        onClick={() =>
                          acceptHandler(candidate.job, candidate.candidate._id, "decline")
                        }
                      >
                    ...
                    <div
                      style={{
                        color: candidate.status === "accepted" ? "green" : "red",
                      }}
                    >
                      {candidate.status === "accepted" ? "Accepted" : "Rejected"}
                      {candidate.status === "accepted" && (
                        <div>{getRoomCodeContent(candidate)}</div>
                      )}
                    </div>
                  )}
                </div>
